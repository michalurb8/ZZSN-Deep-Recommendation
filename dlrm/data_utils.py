...
def convertUStringToDistinctIntsUnique(mat, mat_uni, counts):
    # mat is an array of 0,...,# samples, with each being 3 categorical features
...
def concatCriteoAdData(
    ...
    if memory_map:
        ...
        den_fea = 2  # 2 dense  features
        spa_fea = 3  # 3 sparse features
...
def getCriteoAdData_b(
    ...
    if path.exists(total_file):
        ...
        if criteo_kaggle:
            ...
            # Each line in the file is a sample, consisting of 2 continuous and
            # 3 categorical features (an extra space indicates that feature is
...
def getCriteoAdData(
    ...
    def process_one_file(
        ...
        if dataset_multiprocessing:
            convertDicts_day = [{} for _ in range(3)]

        with open(str(datfile)) as f:
            y = np.zeros(num_data_in_split, dtype="i4")  # 4 byte int
            X_int = np.zeros((num_data_in_split, 2), dtype="i4")  # 4 byte int
            X_cat = np.zeros((num_data_in_split, 3), dtype="i4")  # 4 byte int
            ...
            for k, line in enumerate(f):
                ...
                y[i] = target
                X_int[i] = np.array(line[1:3], dtype=np.int64)
                if max_ind_range > 0:
                    X_cat[i] = np.array(
                        list(map(lambda x: int(x, 16) % max_ind_range, line[3:])),
                        dtype=np.int32,
                    )
                else:
                    X_cat[i] = np.array(
                        list(map(lambda x: int(x, 16), line[3:])), dtype=np.int64
                    )
